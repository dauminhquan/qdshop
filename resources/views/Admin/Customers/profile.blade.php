...
$js = ['assets/js/core/libraries/jasny_bootstrap.min.js',
    'assets/js/plugins/forms/selects/select2.min.js',
    'assets/js/plugins/forms/styling/uniform.min.js','assets/js/plugins/ui/moment/moment.min.js',
    'assets/js/plugins/ui/fullcalendar/fullcalendar.min.js','assets/js/core/app.js','assets/js/plugins/forms/styling/switchery.min.js','assets/js/plugins/forms/styling/switch.min.js','assets/js/plugins/tables/datatables/datatables.min.js','assets/js/plugins/notifications/pnotify.min.js','assets/js/pages/user_profile_tabbed.js','assets/js/plugins/visualization/echarts/theme/limitless.js','assets/js/build/custom.js'];
$html_js = '';
$count = count($js);
for ($i=0; $i < $count ; $i++)  {
		...
		<div class="page-header page-header-default">
			<div class="page-header-content">
				<div class="page-title">
					<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold ">Thông tin khách hàng</span></h4>
				</div>
			...
			<div class="breadcrumb-line">
				<ul class="breadcrumb">
					...
					<li><a href="{{url('admin/customers')}}">Khách hàng</a></li>
					...
					<div class="sidebar-content">
						...
						<div class="content-group">
							<div class="panel-body bg-indigo-400 border-radius-top text-center" style="background-image: url(https://scontent.fhan2-2.fna.fbcdn.net/v/t1.0-9/16684163_661413787362875_6727556996977584083_n.jpg?oh=37cdde42dbb049aee71252976f0ff67d&oe=5A62857C); background-size: contain;">
								...
								<a href="{{route('admin/employees/profile').'?id='.$data->id}}" class="display-inline-block content-group-sm">
									<img id="img_avatar" src="{{$data->avatar}}" class="img-circle img-responsive" alt="" style="height: 150px;width: auto;">
								</a>

								<ul class="list-inline list-inline-condensed no-margin-bottom">
									<li>

										<div tabindex="500" class="btn btn bg-indigo btn-rounded btn-icon btn-file"><i class="icon-image2"></i><input type="file" class="file-input" data-value="{{$data->id}}" id="avatar_customers" data-show-caption="false" data-show-upload="false"></div>
									</li>
									li>
								</ul>
							</div>
							...
							<div class="panel no-border-top no-border-radius-top">
								<ul class="navigation">
									...
									<li class="active"><a href="#profile" data-toggle="tab"><i class="icon-files-empty"></i> Thông tin</a></li>

									<li><a href="#history_order" data-toggle="tab"><i class="icon-files-empty"></i> Lịch sử mua hàng <span class="badge bg-warning-400">{{$len->len}}</span></a></li>

									<li class="navigation-divider"></li>
									<li><a href="login_advanced.html"><i class="icon-switch2"></i> Đăng xuất</a></li>
								</ul>
							</div>
						</div>
						<!-- /user details -->
			...
			<div class="container-detached">
				<div class="content-detached">
					...
					<div class="tab-content">
									...
									<form enctype="multipart/form-data" method="post">
										<div class="form-group">
											<div class="row">
												<div class="col-md-6">
													<label>Họ</label>
													<input name="first_name" type="text" value="{{$data->first_name}}" class="form-control" required>
												</div>
												<div class="col-md-6">
													<label>Tên</label>
													<input name="last_name" type="text" value="{{$data->last_name}}" class="form-control" required>
												</div>
											</div>
										</div>
										...
										<div class="form-group">
											<div class="row">
												<div class="col-md-5">
													<label>Email</label>
													<input name="email_address" readonly type="email" value="{{$data->email_address}}"  class="form-control" required>
												</div>
												<div class="col-md-5">
												...
												<div class="col-md-2">
													...
													<div class="checkbox checkbox-switchery">
														<label>
															<input type="checkbox" class="switchery" name="block" @if($data->block == 1) checked="checked" @endif>
														</label>
													</div>
												</div>
											</div>
										...
										<div class="form-group">
											<div class="row">
												...
												<div class="col-md-6">
													...
													<select class="select-search" data-placeholder="Chọn quận huyện" name="wards" required>
														<option></option>
														@foreach($tinhthanh as $key => $item)
															@if($item['name'] == $data->state_province)
																@foreach($item['districts'] as $keys => $vl)
																	<option value="{{$vl}}" @if($vl == $data->wards) selected @endif>{{$vl}}</option>
																@endforeach
															@endif
														@endforeach
										...
										<div class="form-group">
											<div class="row">
												...
												<div class="col-md-6">
													...
													<input name="business_phone" type="tel" value="{{$data->business_phone}}"  class="form-control">
												</div>
											</div>
										</div>
										...
										<div class="form-group">
											<div class="row">
												<div class="col-md-12">
													<label class="display-block">Thông tin thêm</label>
													<textarea name="notes" class="form-control" rows="8">{{$data->notes}}</textarea>
												</div>
											</div>
										</div>
										<div class="text-right">
											<button type="submit" class="btn btn-primary" name="id" value="{{$data->id}}">Save <i class="icon-arrow-right14 position-right"></i></button>
										</div>
										{{ csrf_field() }}
									</form>
								</div>
						...
						<div class="tab-pane fade" id="history_order">
											...
											<div class="panel-heading">
												<div class="navbar-collapse collapse" id="inbox-toolbar-toggle-multiple">
													...
													<div class="navbar-left">
														<h6 class="navbar-text">Lịch sử mua hàng</h6>
													</div>
													<div class="navbar-right">
														<p class="navbar-text"><span class="label bg-blue heading-text">25 đơn đang đợi</span></p>
														<p class="navbar-text"><span class="label bg-blue heading-text">25 đơn gửi đi</span></p>

													</div>
												</div>
											</div>

											<table class="table table-orders-history">
												<thead>
												<tr>
													<th>Thời gian đặt</th>
													<th>Mã đơn hàng</th>
													<th>Hạn giao hàng</th>
													<th style="min-width: 150px;">Phí ship</th>
													<th style="min-width: 150px;">Thuế</th>
													<th style="min-width: 150px;">Tiền gốc</th>
												...
												@foreach($all_invoice as $item)
															<tr>
																...
																<td>
																	<a href="{{route('admin/invoices/profile').'?id='.$item->id}}" class="text-semibold">#{{$item->id}}</a>
																	<div class="text-muted text-size-small">
											<span class="@switch($item->status_id)
											@case(0) status-mark bg-success position-left @break
											@case(1) status-mark bg-blue position-left @break
											@case(2) status-mark bg-grey position-left @break
											@case(3) status-mark bg-danger position-left @break
											@default status-mark bg-danger position-left
											@endswitch"></span>{{$item->status_name}}
																	</div>
																</td>

																<td style="min-width: 200px;">{{date("d-m-Y", strtotime($item->due_date))}}</td>
																<td style="min-width: 100px;">
																	{{$item->shipping}}<sup>đ</sup>
																</td>
																<td style="min-width: 100px;">
																	{{$item->tax}}<sup>đ</sup>
																</td>
																<td style="min-width: 100px;">
																	{{$item->amount_due}}<sup>đ</sup>
																</td>
																<td style="min-width: 100px;">
																	<h6 class="no-margin text-semibold">{{$item->tax + $item->shipping + $item->amount_due}}<sup>đ</sup></h6>
																</td>
